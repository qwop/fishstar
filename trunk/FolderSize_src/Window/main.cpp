...
LRESULT FSWindow::OnCreate(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled)
{
	...
	//| LVCFMT_FIXED_WIDTH;
	// column.cx = 100;
	column.pszText = _T("名称"); // _T("Name"); // modify by qwop.
	//column.
	ListView_InsertColumn(m_lv, 0, &column);
	column.fmt = LVCFMT_RIGHT;
	column.pszText = _T("大小"); //  _T("Size");  // modify by qwop.
...
void FSWindow::AdjustSizeForList()
{
	...
	/**  add by qwop start.*/
	static UINT FIX_COLUMN_WIDTH = 100;
	ListView_SetColumnWidth( m_lv, 0,g_autosize ? LVSCW_AUTOSIZE : FIX_COLUMN_WIDTH);
	/**  add by qwop end.*/
	...
	// add by qwop.
	ListView_SortItems( m_lv, compareSize, 0 );

	// IDispatch** ppdispath;
	//m_pWebBrowser->get_Container( ppdispath );
...
BEGIN_MSG_MAP(FSWindow)
	...
	COMMAND_ID_HANDLER(ID_AUTOSIZE, OnAutoSize)
END_MSG_MAP()
...
END_SINK_MAP()
	...
	LRESULT OnAutoSize(WORD wNotifyCode, WORD wID, HWND hWndCtl, BOOL& bHandled); // add by qwop
...
//			g_autosize  = mii.fState == MFS_CHECKED;
//			SetMenuItemInfo(hSubMenu, ID_AUTOSIZE, FALSE, &mii); // add by qwop.
...
LRESULT ShellWindowManager::OnAutoSize(WORD wNotifyCode, WORD wID, HWND hWndCtl, BOOL& bHandled)
{
	g_autosize = !g_autosize;
